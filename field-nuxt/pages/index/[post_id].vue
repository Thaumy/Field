...
const it = await useAsyncData(async () => {
  const {requestPostSSR} = await import("@/scripts/data/server/post")
  console.log('invoked requestPostSSR')
  const id = BigInt(route.params.post_id.toString())
  return await requestPostSSR(id)
}).data
